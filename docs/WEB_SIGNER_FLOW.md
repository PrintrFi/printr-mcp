...
TLS is served using the bundled cert for `local.printr.dev` (a real domain whose DNS A record points to `127.0.0.1`). This allows HTTPS origins such as `app.printr.money` to fetch the session API without being blocked by browser Private Network Access (PNA) restrictions. When the cert files are absent (e.g. a stripped CI environment), the server falls back to plain HTTP and `LOCAL_SESSION_ORIGIN` resolves to `http://localhost`.
...
sequenceDiagram
    ...
    rect rgb(20, 40, 70)
        ...
        Note over Agent: deep link = PRINTR_APP_URL/sign?session=TOKEN&api=local.printr.dev:PORT
    end
...
```
{PRINTR_APP_URL}/sign?session={UUID}&api={encodeURIComponent("https://local.printr.dev:{PORT}")}
```
...
| `api` | `https%3A%2F%2Flocal.printr.dev%3A5174` | URL-encoded local server base URL (`LOCAL_SESSION_ORIGIN` + port) |
...
| **CORS wildcard** | Required so the HTTPS sign page can `fetch` from the session server; acceptable because the server is localhost-only |
| **HTTPS via `local.printr.dev`** | The session server uses a bundled TLS cert for `local.printr.dev` (DNS → `127.0.0.1`). This satisfies browser Private Network Access (PNA) checks that block HTTPS→HTTP cross-origin requests (e.g. Brave with shields up). The private key is intentionally distributed with the package — the domain resolves only to localhost, so possession of the key confers no advantage over the network. |
...
```sh
# SVM session (default)
bun run dev:stub-session

# EVM session (Base)
bun run dev:stub-session --evm
```

Override the app base URL via env var:

```sh
PRINTR_APP_URL=http://localhost:3000 bun run dev:stub-session
```
